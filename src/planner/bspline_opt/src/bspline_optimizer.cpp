                                                                       ...
                                                                       bool flag_first_init /*= true*/)
{
    ...
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
        if (i == 0)  // first segment
        {
            ...
            {
                id_up_bound =
                    (int)(((segment_ids[0].second + segment_ids[1].first) - 1.0f) / 2);  // id_up_bound : -1.0f fix()
            }
            else
            {
    ...
    for (int i = 0; i < q.cols() - 1; i++)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            if (vi(j) > max_vel_)
            {
                ...
                // cout << vi(j) << endl;
                cost +=
                    pow(vi(j) - max_vel_, 2) * ts_inv2;  // multiply ts_inv3 to make vel and acc has similar magnitude
...
bool BsplineOptimizer::check_collision_and_rebound(void)
{
    ...
    if (flag_new_obs_valid)
    {
        ...
        {
            // step 1
            for (int j = segment_ids[i].first; j <= segment_ids[i].second; ++j) cps_.flag_temp[j] = false;
...
bool BsplineOptimizer::refine_optimize()
{
    ...
    do
    {
        ...
        int result = lbfgs::lbfgs_optimize(variable_num_, q, &final_cost, BsplineOptimizer::costFunctionRefine, NULL,
                                           NULL, this, &lbfgs_params);
        if (result == lbfgs::LBFGS_CONVERGENCE || result == lbfgs::LBFGSERR_MAXIMUMITERATION ||
            result == lbfgs::LBFGS_ALREADY_MINIMIZED || result == lbfgs::LBFGS_STOP)
        {
            // pass

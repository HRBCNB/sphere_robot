...
class GlobalTrajData  // 全局轨迹
{
   private:
   public:
    PolynomialTraj global_traj_;         // 全局多项式轨迹（基准参考轨迹）
    vector<UniformBspline> local_traj_;  // 局部 B-spline 轨迹序列：
                                         //   local_traj_[0] = 位置样条
                                         //   local_traj_[1] = 速度样条（由位置样条求导）
                                         //   local_traj_[2] = 加速度样条（由速度样条求导）

    double global_duration_;                    // 全局轨迹总时长（秒）
    ros::Time global_start_time_;               // 全局轨迹起始时间（ROS 时间）
    double local_start_time_, local_end_time_;  // 当前局部段在全局时间轴上的开始/结束（秒）
    double time_increase_;                      // 插入局部段后对全局时间轴的累计偏移（秒）
    double last_time_inc_;                      // 最近一次插入的时间增量（秒），用于切换补偿
    double last_progress_time_;                 // 最近进度时间标记（用于统计/超时判断）
...
struct LocalTrajData
{
    ...
    int traj_id_;               // 段数
    double duration_;           // 该段时间
    double global_time_offset;  // This is because when the local traj finished and is going to switch back to the
                                // global traj, the global traj time is no longer matches the world time.
    ros::Time start_time_;
    Eigen::Vector3d start_pos_;
    UniformBspline position_traj_, velocity_traj_, acceleration_traj_;  // 该段轨迹的B样条

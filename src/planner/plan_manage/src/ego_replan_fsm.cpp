...
void EGOReplanFSM::planGlobalTrajbyGivenWps()
{
    ...
    // 生成全局参考轨迹
    bool success =
        planner_manager_->planGlobalTrajWaypoints(odom_pos_, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(), wps,
...
void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent& e)
{
    ...
    {
        if (t_cur < t_2_3 && t >= t_2_3)  // If t_cur < t_2_3, only the first 2/3 partition of the trajectory is
...
void EGOReplanFSM::getLocalTarget()
{
    ...
    {
        // local_target_vel_ = (end_pt_ - init_pt_).normalized() * planner_manager_->pp_.max_vel_ * (( end_pt_ -
        // local_target_pt_ ).norm() /
        // ((planner_manager_->pp_.max_vel_*planner_manager_->pp_.max_vel_)/(2*planner_manager_->pp_.max_acc_))); cout
        // << "A" << endl;
